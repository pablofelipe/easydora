...
| Auth | Spring Boot + PostgreSQL + JWT + Outbox | 8081 | Implemented | 26 tests — unit + `*IT` (Outbox, real Postgres/RabbitMQ), including 3 contract tests covering `user.registered`/`user.verified`/`jwt.created` ([ADR-0002](docs/adr/0002-json-schema-contract-testing.md)) and a Progress Watchdog backing its `/health/liveness` signal ([ADR-0038](docs/adr/0038-infrastructure-startup-resilience.md)'s Update) |
| Products | Spring Boot + PostgreSQL + RabbitMQ | 8082 | Implemented | 29 unit tests, including 6 contract tests covering `product.created`/`product.updated`/`product.deleted`/`user.verified`/`jwt.created` ([ADR-0002](docs/adr/0002-json-schema-contract-testing.md)) and a Progress Watchdog backing its `/health/liveness` signal ([ADR-0038](docs/adr/0038-infrastructure-startup-resilience.md)'s Update) |
| Inventory | Go + PostgreSQL + RabbitMQ + Outbox | 8083 | Implemented | 32 tests — 25 unit + 7 integration (real Postgres/RabbitMQ, includes concurrency via `go test -race`), including 7 contract tests covering `product.*`/`stock.*` ([ADR-0002](docs/adr/0002-json-schema-contract-testing.md)) and a `NotifyClose`-based reconnect supervisor proven against a real broker restart ([ADR-0038](docs/adr/0038-infrastructure-startup-resilience.md)'s Update) |
| Orders | Spring Boot + PostgreSQL + RabbitMQ + Outbox | 8084 | Implemented | 109 tests — unit + `*IT` (real Postgres/RabbitMQ), including 4 covering self-purchase prevention, 31 covering the order fulfillment lifecycle (ship/deliver, single-source-of-truth transitions, the `previousState` fix), 6 covering optimistic locking on `Order`, 23 covering the payment compensation saga ([ADR-0034](docs/adr/0034-payment-compensation-saga.md)), Outbox coverage for all four publishes ([ADR-0037](docs/adr/0037-consolidated-outbox-pattern-specification.md)), 13 contract tests ([ADR-0002](docs/adr/0002-json-schema-contract-testing.md)), and a Progress Watchdog backing its `/health/liveness` signal ([ADR-0038](docs/adr/0038-infrastructure-startup-resilience.md)'s Update) |
| Billing | Spring Boot + PostgreSQL + RabbitMQ + JWT + Outbox | 8085 | Implemented | 57 tests — unit + `*IT` (real Postgres/RabbitMQ), including 6 covering the deterministic payment provider and its single-creation-path guarantee, 4 covering optimistic locking on `Payment`, 7 covering the payment compensation saga ([ADR-0034](docs/adr/0034-payment-compensation-saga.md)), Outbox coverage for all four publishes ([ADR-0037](docs/adr/0037-consolidated-outbox-pattern-specification.md)), 6 contract tests ([ADR-0002](docs/adr/0002-json-schema-contract-testing.md)), and a Progress Watchdog backing its `/health/liveness` signal ([ADR-0038](docs/adr/0038-infrastructure-startup-resilience.md)'s Update) |
| Notification | FastAPI + PostgreSQL + RabbitMQ | 8086 | Implemented | 63 tests — 54 unit + 9 integration (real Postgres/RabbitMQ/auth-service), including 6 contract tests covering `order.created`/`order.status-changed`/`jwt.created` ([ADR-0002](docs/adr/0002-json-schema-contract-testing.md)) and an explicit heartbeat + Progress Watchdog proven to resume consuming after a real broker-forced disconnect ([ADR-0038](docs/adr/0038-infrastructure-startup-resilience.md)'s Update) |
...
- [x] **Opened 2026-07-15, closed 2026-07-15 (Architectural note).** The
      ...
      policy for already-published `outbox_events` rows (they accumulate
      ...
      revisiting; CDC explicitly rejected (would reintroduce
...
- [x] **Opened 2026-07-15, closed 2026-07-15 (Architectural note).** The
      ...
      wipes its cache; recovery is the user logging in again) had never
      had its own ADR — only mentioned as an aside inside ADR-0027 and
      ADR-0028. A second review also found a previously undocumented
      asymmetry: no cache entry expires on its own JWT's `expiresIn`,
      only on restart. See [ADR-0039](docs/adr/0039-jwt-broadcast-cache-restart-and-ttl.md):
...
- [x] **Opened 2026-07-15, closed 2026-07-15 (Architectural note).** No
      ...
      accepted residual risk (documented above and provable only by unit
      test) with zero runtime observability. Closed by ADR-0036's
      2026-07-15 Update: a sixth business counter,
      `inventory_idempotent_duplicate_detected_total{operation}`,
      incremented exactly once per cache hit on `ReserveStock` or
      `ReleaseStock`. The JWT broadcast cache's equivalent gap (any
      service) was tracked alongside the `expiresIn` TTL ADR-0039 had
      decided but not yet implemented — both closed the same day, see the
      two items below.
- [x] **Opened 2026-07-15, closed 2026-07-15.** The `expiresIn`-based TTL
      ...
      `expiresIn` for the first time (previously absent, unlike `UserEvent`,
      which already had both); `notification-service`'s
      `_cache_jwt_created` now reads every field the shared jwt-created
      schema declares. Every service gained a `JwtAuthenticationFilterExpiryTest`
      (or the Python equivalent in `tests/test_auth.py`) proving hit,
      expired, and never-cached (the restart scenario) as three distinct
      outcomes — not just "at least one" service, all four. TDD throughout;
      full suite per service, no regressions: 48/48 (`billing-service`),
      22/22 (`products-service`), 99/99 (`orders-service`), 48/48
      non-integration (`notification-service`).
- [x] **Opened 2026-07-15, closed 2026-07-15.** No metric distinguished a
      ...
      four broadcast-JWT caches — the same kind of runtime-observability
      gap ADR-0036's first 2026-07-15 Update closed for
      `inventory-service`'s idempotency cache. Closed by ADR-0036's second
      2026-07-15 Update: `jwt_cache_lookup_total{outcome}` (`hit`/`miss`/
      `expired`) in all four services, incremented at the same point the
      TTL check above already runs. The three Java services use
      Micrometer via a constructor-injected `ObjectProvider<MeterRegistry>`
      (not a direct `MeterRegistry` dependency), specifically so
      `JwtAuthenticationFilter` still constructs cleanly inside a
      `@WebMvcTest` slice — which doesn't autoconfigure a real
      `MeterRegistry` bean — falling back to a private, unscraped
      `SimpleMeterRegistry` in that case, with zero changes needed to any
      existing controller test. `notification-service` uses
      `prometheus_client.Counter`, the same mechanism
      `notifications_sent_total` already uses.
- [x] **Opened 2026-07-17, closed 2026-07-17 (Architectural note).** Docker
      ...
      `kind` chosen over Minikube/k3d on technical grounds. Docker Compose
      remains the recommended environment for local development. A
      2026-07-17 Update adds the frontend, reusing the same image already
      built for Docker Compose unmodified (its `VITE_GATEWAY_URL` build-time
      value already matches the Gateway's address under `kind`). See
...
- [x] **Opened 2026-07-17, closed 2026-07-17.** Generating test data against
      ...
      client's existing connection dead, and `inventory-service` (Go) never
      ...
      rather than assuming it (`amqp-client`'s automatic connection/topology
      recovery already handles this), and rejected both a PersistentVolume
      for RabbitMQ (only helps queues survive, not client reconnection) and
      a liveness-probe-only fix (not portable to Docker Compose, which
      doesn't restart a container on a failed healthcheck). Closed the one
      real code gap with a `NotifyClose`-based reconnect supervisor in
      `inventory-service`, gave `notification-service`'s `pika` consumer an
      explicit heartbeat, added a Progress Watchdog to all six services
      feeding each one's `/health/liveness` (fed by reconnect attempts and
      messages processed, deliberately never by instantaneous broker
      reachability), fixed a found-along-the-way `RabbitMQHealthIndicator`
      exchange-type bug, and added `restart: on-failure` to every
      `docker-compose.yml` service. See
