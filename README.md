...
- [x] **Opened 2026-07-04 (Low).** Whether `JWT_SECRET`/`app.jwt.secret`
      ...
      untouched.
- [x] **Opened 2026-07-09 (Low).** `notification-service`'s
      ...
      Closed (see [ADR-0022](docs/adr/0022-notification-service-consumption-resilience.md)'s
      2026-08-03 Update): confirmed this posed no real idempotency risk
      (both business functions either never raise, or raise before any
      persistence happens), so the actual gap was purely observability —
      a retry or dead-lettering was invisible outside this file's own log
      lines. Added `notification_retry_total{outcome}`, a Prometheus
      counter, plus a Grafana panel on the `EasyDora / Resilience`
      dashboard. Neither business function changed.
- [x] **Opened 2026-07-15 (Low).** The four Spring services' tolerance of
      ...
      [ADR-0038](docs/adr/0038-infrastructure-startup-resilience.md).
      Closed by its 2026-08-03 Update: a new `boot-race` CI job (one
      matrix entry per Spring service) owns this now, deliberately
      without a `services:` block so it can start the app before
      Postgres/RabbitMQ exist, then bring both up late and assert
      recovery. Evaluated and rejected embedded Postgres as unnecessary —
      `docker run`, already used elsewhere in this CI file, reproduces the
      same race with no new dependency.
- [x] **Opened 2026-07-15 (Low).** `notification-service`'s unbounded
      RabbitMQ reconnect retry is inconsistent with its own bounded
      Postgres retry. See
      [ADR-0038](docs/adr/0038-infrastructure-startup-resilience.md).
      Closed by its second 2026-08-03 Update: confirmed the steady-state
      reconnect path (after a first successful connection) is
      deliberately unbounded everywhere in this project, cross-language —
      the actual gap was that the very first boot-time connection attempt
      was never structurally separated from it, unlike `inventory-service`
      (Go). Added `BOOT_MAX_ATTEMPTS = 10`, matching `ensure_schema`'s own
      Postgres bound; exhausting it now raises instead of retrying
      forever, letting the container crash and restart instead of
      reporting healthy forever having never connected once.
- [x] **Opened 2026-08-02 (Low).** Outbox-mediated publishes
      (`orders-service`, `inventory-service`, `billing-service`) don't
      yet carry a trace across their write-to-publish gap, unlike
      CorrelationId's own envelope trick for the same gap. Closed by its
      2026-08-03 Update: the envelope gained a `traceparent` field in
      both languages, captured at write time and restored as the publish
      call's producer span parent — `auth-service` included, via the
      same shared `correlation-commons` codec. A first version, built on
      raw OpenTelemetry API and passing every unit test, was caught by
      live validation against a real Jaeger trace: outbox publishes
      chained correctly among themselves but never linked back to the
      original HTTP request, since Spring AMQP's instrumentation reads
      Micrometer's own current-span tracking, not OTel's raw context.
      Corrected to build on Micrometer's own `Tracer`/`Propagator`
      instead. `OutboxTraceparentTest` now builds a real Micrometer OTel
      bridge (no Jaeger needed) to prove genuine parent/child linkage,
      not just matching trace ids. Full live-Jaeger re-confirmation is
      this project's next Docker Compose validation pass. See
      [ADR-0024](docs/adr/0024-distributed-tracing-via-propagated-identifiers.md)'s
      2026-08-02 Update.
