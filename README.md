...
| Billing | Spring Boot + PostgreSQL + RabbitMQ + JWT + Outbox | 8085 | Implemented | 54 tests — unit + `*IT` (real Postgres/RabbitMQ), including 6 covering the deterministic payment provider and its single-creation-path guarantee, 4 covering optimistic locking on `Payment`, 7 covering the payment compensation saga ([ADR-0034](docs/adr/0034-payment-compensation-saga.md)), Outbox coverage for all four publishes ([ADR-0037](docs/adr/0037-consolidated-outbox-pattern-specification.md)), and 6 contract tests ([ADR-0002](docs/adr/0002-json-schema-contract-testing.md)) |
