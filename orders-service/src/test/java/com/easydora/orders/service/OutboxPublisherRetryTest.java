...
/**
 * Unit-level proof that orders-service's OutboxPublisher never loses a row
 * when the broker is unreachable at poll time -- mirrors auth-service's
 * OutboxPublisherRetryTest (ADR-0037's consolidated specification). Both
 * collaborators are mocked: this is about the poller's own retry logic, not
 * broker/DB wiring.
 */
