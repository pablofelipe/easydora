 ...
 * (UserEventsConsumer.handleUserVerified(Long userId, ...)) must conform to
...
class UserVerifiedConsumerContractTest {
    ...
    void consumedEventConformsToSharedSchema() throws IOException {
        ...
        assertThat(errors)
                .withFailMessage("orders-service's user.verified consumption violates shared schema: %s", errors)
                .isEmpty();
    }
}
