 ...
 * service -- OrderService writes the row in the same transaction as its own
 * domain change and never talks to RabbitMQ directly, so a save failure can
 * never leave an event published without the matching data.
 *
 ...
 * design as auth-service's OutboxPublisher and inventory-service's Go
 * equivalent -- see ADR-0037 for the consolidated specification.
 */
@Component
