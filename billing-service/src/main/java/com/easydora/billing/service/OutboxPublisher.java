 ...
 * This is the only place that publishes an outbox-backed event for this
 * service -- PaymentService writes the row in the same transaction as its
 * own domain change and never talks to RabbitMQ directly, so a save failure
 * can never leave an event published without the matching data.
 *
 ...
 * retried on the next poll -- at-least-once delivery, never lost. Same
 * design as auth-service's/inventory-service's/orders-service's
 * OutboxPublisher -- see ADR-0037 for the consolidated specification.
 */
@Component
